...
We will then review examples of industry ecosystems where this kind of identity meta-platform will address existing interoperability problems and provide real value, today.
...
This "free" value transfer across networks within the same context is what is normally considered when using the term cooperation. By context we mean similar types of products and services. We call this intra-contextual cooperation that results in intra-contextual value transfer between networks. This focus of this paper, however, is to address the question: What about cooperating networks where participants are "free" to transfer value between networks from different contexts? In other words the networks are providing different, non-interoperable products and services? This is what we call inter-contextual (or more easily pronounced trans-contextual) network value transfer. Trans-contextual value transfer, when possible, has the potential to remove barriers to cooperation among perceived competitors. This poses the question, Is trans-contextual value transfer even possible? and if so, to what extent is that value transfer "free"?  Emphasizing this categorical distinction (intra vs inter context) and a holistic accounting for overcoming non-interoperability costs in strategic planning might reduce barriers to cooperation. In the next section we will talk about value transfer that may occur between contexts and context-specific products.
...
Most importantly, a decentralized identity and reputation system enables trust as per the lowering of trust transaction costs to be transitive between contexts to some degree. This degree of cooperative network-of-network effects for trust transaction costs can be quantified by adding a transitivity factor to the equations above. This transitive formulation is provided in detail in [[1]].
