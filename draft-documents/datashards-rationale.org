...
The foundational operations for a content store endpoint is:
 ...
 - POST/PUT operation: Accepts as its body a shard of no more and no
   less than 32 kilobytes.
   It returns the Shard URN that it thinks this data is represented
   by that data.
 - GET operation: Is queried for a Shard URN and in its response body
